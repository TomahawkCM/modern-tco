...
const averageSpending = transactions
...
<AreaChart data={[...historicalData, { month: 'Next', value: nextMonthForecast, isDashed: true }]} />
```
  ...
  content={(props) => (
    <div className="bg-white p-3 rounded-lg shadow-lg border">
      ...
      <p className="text-sm text-gray-600">
        {props.value} (23% higher than last month)
      </p>
...
<div {...handlers} className="relative">
  ...
  {showDelete && (
    <button
      className="absolute right-0 inset-y-0 w-20 bg-red-600 text-white"
...
const ys = tf.oneHot(transactions.map(tx => categoryIndex[tx.category]), categories.length);
...
{selectedIds.length > 0 && (
  <div className="fixed bottom-4 inset-x-4 bg-white rounded-lg shadow-xl p-4">
...
function ShortcutRow({ keys, description }) {
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <div className="flex gap-1">
        {keys.map(key => (
          <kbd key={key} className="px-2 py-1 bg-gray-100 border border-gray-300 rounded text-sm font-mono">
            {key}
          </kbd>
        ))}
      </div>
