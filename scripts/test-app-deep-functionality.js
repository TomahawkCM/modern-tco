...
async function testDeepFunctionality() {
  ...
  try {
    ...
    const stats = await page
      .locator('[class*="stat"], [class*="metric"], [class*="number"]')
      .count();
    ...
    for (const bar of progressBars.slice(0, 3)) {
      const value =
        (await bar.getAttribute("aria-valuenow").catch(() => null)) ||
    ...
    const practiceButtons = await page
      .locator("button:has-text(/practice/i), a:has-text(/practice/i)")
      .count();
    const examButtons = await page.locator("button:has-text(/exam/i), a:has-text(/exam/i)").count();
    const quizButtons = await page
      .locator("button:has-text(/quiz|test/i), a:has-text(/quiz|test/i)")
      .count();
    ...
    // Check for settings
    const settingsElements = await page
      .locator('[class*="settings"], button:has-text(/settings/i)')
      .count();
    console.log(`  ✅ Settings Elements: ${settingsElements}`);

    // Check for theme toggle
    const themeToggle = await page
      .locator('[class*="theme"], [class*="dark"], button:has-text(/theme/i)')
      .count();
    ...
    const authElements = await page
      .locator('[class*="auth"], [class*="login"], [class*="logout"]')
      .count();
  ...
  console.log(
    `  • Assessment Features: ${(results.features.assessments?.practiceButtons || 0) + (results.features.assessments?.examButtons || 0)} buttons`
  );
  console.log(`  • User Settings: ${results.features.userSettings?.userElements || 0} elements`);
  console.log(
    `  • Data Visualization: ${(results.features.visualization?.svg || 0) + (results.features.visualization?.canvas || 0)} elements`
  );
  ...
  console.log(
    "  🟢 Comprehensive LMS features including modules, assessments, and progress tracking"
  );
  ...
  require("fs").writeFileSync("/tmp/tco-deep-test-results.json", JSON.stringify(results, null, 2));
