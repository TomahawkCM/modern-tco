...
async function testFullApplication() {
  ...
  try {
    ...
    logTest(
      "Navigation",
      "Sidebar",
      sidebarExists ? "PASS" : "WARN",
      sidebarExists ? "Found" : "Not visible on initial load"
    );

    // Check for main content area
    const mainExists =
      (await page.locator('main, [class*="main"], [class*="content"]').count()) > 0;
    ...
    const practiceElements = await page
      .locator("text=/Practice|Exam|Test|Quiz|Assessment/i")
      .count();
    logTest(
      "TCO Features",
      "Assessment Elements",
      practiceElements > 0 ? "PASS" : "WARN",
    ...
    logTest(
      "TCO Features",
      "Progress Indicators",
      progressElements >= 0 ? "PASS" : "WARN",
      `Found: ${progressElements}`
    );

    // Check for domain coverage
    const domainText = await page
      .locator("text=/Domain|Asking Questions|Refining|Navigation/i")
      .count();
    logTest(
      "TCO Features",
      "Domain References",
      domainText >= 0 ? "PASS" : "WARN",
      `Found: ${domainText}`
    );
    ...
    logTest(
      "Interactions",
      "Clickable Buttons",
      clickableButtons.length > 0 ? "PASS" : "WARN",
    ...
    const mobileMenuExists =
      (await page
        .locator('[class*="mobile"], [class*="hamburger"], [class*="menu-toggle"]')
        .count()) > 0;
    logTest(
      "Responsive",
      "Mobile Menu",
      mobileMenuExists ? "PASS" : "WARN",
    ...
    logTest(
      "Accessibility",
      "Image Alt Text",
      images === 0 || imagesWithAlt > 0 ? "PASS" : "WARN",
      `${imagesWithAlt}/${images} images have alt text`
    );

    // Check for semantic HTML
    const semanticElements = await page
      .locator("header, nav, main, footer, section, article, aside")
      .count();
    logTest(
      "Accessibility",
      "Semantic HTML",
      semanticElements > 3 ? "PASS" : "WARN",
...
function generateSummaryReport() {
  ...
  console.log(
    `  ✅ Passed: ${testResults.summary.passed} (${Math.round((testResults.summary.passed / testResults.summary.total) * 100)}%)`
  );
  console.log(
    `  ⚠️  Warnings: ${testResults.summary.warnings} (${Math.round((testResults.summary.warnings / testResults.summary.total) * 100)}%)`
  );
  console.log(
    `  ❌ Failed: ${testResults.summary.failed} (${Math.round((testResults.summary.failed / testResults.summary.total) * 100)}%)`
  );
  ...
  fs.writeFileSync("/tmp/tco-test-results.json", JSON.stringify(testResults, null, 2));
