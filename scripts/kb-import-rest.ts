...
async function main(): Promise<void> {
  ...
  console.log(
    `• Discovered ${modules.length} module(s), ${lessons.length} lesson(s) from docs/KB/lessons`
  );
  ...
  const lessonProbe = await supabase
    .from("kb_lessons")
    .select("count", { count: "exact", head: true });
  if (lessonProbe.error) {
    console.log(
      "• public.kb_lessons not available — skipping lesson import (create table or expose schema to enable)."
    );
  } else {
    ...
    for (const lesson of lessons) {
      ...
      const res = await supabase
        .from("kb_lessons")
        .upsert(payload, { onConflict: "module_id,slug" });
      if (res.error) {
