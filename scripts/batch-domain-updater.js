...
async function processBatch(batchQuestions, batchNumber, totalBatches) {
  ...
  try {
    ...
    console.log(
      `✅ Batch ${batchNumber} completed: ${successful.length} success, ${failed.length} failed`
    );
...
async function validateDistribution() {
  ...
  Object.entries(expectedDistribution).forEach(([domain, expectedPercent]) => {
    ...
    console.log(
      `  ${status} ${domain}: ${actualPercent}% (target: ${expectedPercent}%, deviation: ${deviation.toFixed(1)}%)`
    );
  });
}
