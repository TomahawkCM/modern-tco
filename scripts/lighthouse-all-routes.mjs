...
function runLighthouse(url, preset, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    // Configure for desktop vs mobile
    // IMPORTANT: Lighthouse CLI does NOT accept --preset=mobile
    // Valid presets: 'desktop', 'perf', 'experimental'
    // For mobile emulation, use --form-factor=mobile instead
    if (preset === "desktop") {
      args.push("--preset=desktop");
    } else if (preset === "mobile") {
      // Mobile uses form-factor instead of preset (fixed: was --preset=mobile)
