...
const stageIcons: Record<string, React.ReactNode> = {
  loading: <FileText className="h-5 w-5 text-blue-500 animate-pulse" />,
  rendering: <Eye className="h-5 w-5 text-purple-500 animate-pulse" />,
  preprocessing: <Scan className="h-5 w-5 text-orange-500 animate-pulse" />,
  ocr: <FileSearch className="h-5 w-5 text-cyan-500 animate-pulse" />,
  parsing: <Loader2 className="h-5 w-5 text-green-500 animate-spin" />,
...
export function PDFOCRProgress({
  ...
  // Track elapsed time
  useEffect(() => {
    if (!progress || progress.stage === 'complete' || progress.stage === 'error' || progress.stage === 'cancelled') {
      return;
    }
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between pt-2">
          {['loading', 'rendering', 'preprocessing', 'ocr', 'parsing', 'complete'].map((stage, index) => {
            const stages = ['loading', 'rendering', 'preprocessing', 'ocr', 'parsing', 'complete'];
              ...
              >
                <div
                  className={`
                    ...
                    ${isActive && !isError ? 'bg-primary text-primary-foreground ring-2 ring-primary/30' : ''}
                ...
                <span className="text-[10px] text-muted-foreground mt-1 text-center">
                  {stage === 'preprocessing' ? 'Enhance' : stage.charAt(0).toUpperCase() + stage.slice(1)}
                </span>
              </div>
            );
          })}
    ...
    <div className="flex items-center gap-3 text-sm">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <span className="font-medium truncate">{progress.message}</span>
          <span className="text-muted-foreground ml-2">{Math.round(progress.progress)}%</span>
        </div>
