...
export function AccessibilitySettingsPanel() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-lg border-2 border-teal-200 bg-gradient-to-r from-teal-50 to-blue-50 p-6 shadow-lg dark:border-teal-800 dark:from-teal-950 dark:to-blue-950">
        <div className="mb-4 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">{t("seniorsMode.description")}</p>
          </div>
        </div>
      ...
      <div className="rounded-lg bg-card p-6 shadow">
        <div className="mb-4">
          ...
          <p className="text-sm text-muted-foreground">{t("themeMode.description")}</p>
        </div>
      ...
      <div className="rounded-lg bg-card p-6 shadow">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">{t("reducedMotion.description")}</p>
          </div>
          <Switch
      ...
      <div className="rounded-lg bg-muted p-4">
        <h4 className="mb-2 font-semibold text-foreground">{t("infoBox.title")}</h4>
...
function SeniorsModePresetsSection({ t }: SeniorsModePresetsSectionProps) {
  ...
  return (
    <div className="mt-4 border-t border-teal-200 pt-4 dark:border-teal-800">
      <p className="mb-3 text-sm font-medium text-muted-foreground">{t("presets.title")}</p>
      <div className="grid grid-cols-4 gap-2">
        {presets.map((preset) => (
          <button
            key={preset}
            onClick={() => applyPreset(preset)}
            className={`min-h-[52px] rounded-lg p-3 text-center transition-all ${
              currentPreset === preset
                ? "bg-teal-600 text-white shadow-md"
                : "border border-gray-200 bg-white text-foreground hover:bg-teal-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
            }`}
            aria-pressed={currentPreset === preset}
          >
            <div className="text-sm font-semibold">{t(presetKeys[preset].label)}</div>
            <div
              className={`mt-0.5 text-xs ${currentPreset === preset ? "text-teal-100" : "text-muted-foreground"}`}
            >
