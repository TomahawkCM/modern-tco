              ...
              {week.map((day, dayIndex) => {
                ...
                return (
                  <div
                    ...
                    className={`aspect-square ${colorClass} group relative flex cursor-pointer flex-col items-center justify-center rounded-lg transition-all hover:ring-2 hover:ring-teal-500`}
                    onMouseEnter={() => setHoveredDay(day)}
                    onMouseLeave={() => setHoveredDay(null)}
                  >
                    <span className="text-xs font-medium text-gray-800">{day.date.getDate()}</span>
        ...
        <div className="mt-4 grid grid-cols-3 gap-4 border-t border-gray-200 pt-4">
          <div>
            ...
            <div className="text-lg font-bold text-gray-900">
              $
              {Array.from(dailySpending.values())
          ...
          <div>
            ...
            <div className="text-lg font-bold text-gray-900">
              {(() => {
                const busiest = Array.from(dailySpending.values()).reduce(
                  (max, d) => (d.amount > max.amount ? d : max),
                  { date: new Date(), amount: 0, transactionCount: 0 }
                );
                return busiest.amount > 0 ? dayNames[busiest.date.getDay()] : "-";
              })()}
            </div>
          </div>
          <div>
            ...
            <div className="text-lg font-bold text-gray-900">
              $
              {(
                Array.from(dailySpending.values()).reduce((sum, d) => sum + d.amount, 0) /
                daysInMonth
              ).toFixed(2)}
            </div>
          </div>
