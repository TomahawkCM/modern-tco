  ...
  return (
    <Card className="flex h-full flex-col">
      ...
      {relatedResources && relatedResources.length > 0 && (
        <div className="border-t px-4 py-2">
          <p className="mb-2 text-xs text-muted-foreground">Related resources:</p>
          <div className="flex flex-wrap gap-2">
            {relatedResources.map((resource, index) => (
              ...
              >
                {resource.type === "module" && <Book className="h-3 w-3" />}
                {resource.type === "video" && <Video className="h-3 w-3" />}
                {resource.type === "lab" && <TestTube className="h-3 w-3" />}
                {resource.type === "practice" && <FileText className="h-3 w-3" />}
                {resource.title}
              </a>
            ))}
          </div>
