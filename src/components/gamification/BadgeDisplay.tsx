...
export default function BadgeDisplay({
  ...
  return (
    <Card
      className={`relative overflow-hidden transition-all ${classes.container} ${earned ? tierBorderColors[badge.tier] : "border-gray-700"} ${earned ? "border-2" : "border"} ${!earned ? "opacity-60 grayscale" : ""} ${onClick ? "cursor-pointer hover:scale-105" : ""} ${earned ? `bg-gradient-to-br ${tierGradients[badge.tier]}` : "bg-muted/30"} `}
      onClick={onClick}
    >
      {/* Earned Indicator */}
      {earned && (
      ...
      <div className="flex flex-col items-center gap-3">
        <div
          className={`rounded-full p-4 ${earned ? tierGradients[badge.tier] : "bg-card/50"} border-2 ${earned ? tierBorderColors[badge.tier] : "border-gray-600"} `}
        >
