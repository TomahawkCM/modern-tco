...
async function getDeviceFingerprint(): Promise<string> {
  try {
    ...
    const fallbackStr = Array.from(fallback)
 ...
 */
export async function getOrCreateEncryptionKey(password?: string): Promise<CryptoKey> {
  try {
 ...
 */
export async function decryptAmount(encrypted: string, iv: string): Promise<number> {
  if (!encrypted || !iv) {
    return 0;
  }
 ...
 */
export async function encryptTransaction(transaction: {
  description?: string;
  amount?: number;
}): Promise<{
