 ...
 * and string similarity. For enhanced AI-powered detection, see
 * `@/lib/ai/smart-duplicate-detection.ts` (opt-in, requires AIProvider).
 */
...
export async function detectDuplicates(
  ...
  if (useSmartDetection) {
    try {
      ...
      await detectDuplicatesEnhanced(newTransactions, existingTransactions as any[], true);
      return; // Smart detection handles everything
    } catch (error) {
      console.warn(
        "[DuplicateDetector] Smart detection unavailable, falling back to basic matching:",
        error
      );
