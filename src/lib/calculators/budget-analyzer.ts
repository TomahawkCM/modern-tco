 ...
 */
export function calculateBudgetAnalysis(input: BudgetAnalyzerInput): BudgetAnalyzerResult {
  ...
  const totalSavings = roundToCents(savings.actualAmount);
  ...
  const recommendations = generateRecommendations(needs, wants, savings, monthlyNetIncome);
...
function createBucketAnalysis(
  ...
  const actualAmount = roundToCents(data.total);
  const actualPercent = income > 0 ? roundToCents((actualAmount / income) * 100) : 0;
  const variance = roundToCents(actualAmount - targetAmount);
  const variancePercent = roundToCents(actualPercent - targetPercent);
 ...
 */
function inferBucket(categoryName: string): BudgetBucket {
...
function generateRecommendations(
  ...
  if (needs.isOverBudget && !wants.isOverBudget) {
    const couldRedirect = roundToCents(wants.targetAmount - wants.actualAmount);
    if (couldRedirect > 0) {
      recommendations.push(
