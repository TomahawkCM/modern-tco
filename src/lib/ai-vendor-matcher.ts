...
export async function aiFindMatchingVendorTransactions(
  ...
  console.log(
    `[AI Vendor Match] Searching for "${currentVendor}" in ${recentTransactions.length} transactions`
  );
...
OUTPUT FORMAT: Return ONLY comma-separated numeric IDs of matching transactions (e.g., "0,5,12,23") or "none" if no matches.`,
      ...
      console.log(
        `[AI Vendor Match] Found ${matches.length} matches:`,
        matches.map((m) => extractVendorName(m.description))
      );
    ...
    const isTimeoutError =
      errorMessage.includes("timeout") || errorType === "aborted" || error?.name === "AbortError";
    ...
    if (isConnectionError || isTimeoutError) {
      console.warn(
        "[AI Vendor Match] Connection/timeout error - falling back to string matching:",
        errorMessage
      );
