...
export async function markSectionProgress(opts: {
  ...
  const { error } = await (client as any).from("study_progress").upsert(
    {
      user_id: userId,
...
export async function upsertUserModuleProgress(opts: { userId: string; moduleId: string }) {
  const client = supabaseAdmin || supabase;
  ...
  const status =
    completed && total ? (completed >= total ? "completed" : "in_progress") : "in_progress";

  const { error: upsertErr } = await (client as any).from("user_module_progress").upsert(
    {
      user_id: opts.userId,
