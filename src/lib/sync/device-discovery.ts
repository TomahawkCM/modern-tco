...
import type { PairingQRData, PairedDevice, DeviceIdentity } from "./types";
 ...
 */
export function savePairedDevice(
  device: Omit<PairedDevice, "id" | "createdAt" | "updatedAt">
): PairedDevice {
  const devices = getPairedDevices();
 ...
 */
export async function startPairingSession(ip?: string, port?: number): Promise<PairingQRData> {
