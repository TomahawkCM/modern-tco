      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="text-sm font-medium text-muted-foreground">{t("stats.active")}</h3>
                <TrendingUp className="h-5 w-5 text-teal-500" />
              </div>
              <p className="text-3xl font-bold text-foreground">{stats.totalActive}</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("stats.inactive", { count: stats.totalInactive })}
              </p>
            </div>
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="text-sm font-medium text-muted-foreground">{t("stats.monthly")}</h3>
                <DollarSign className="h-5 w-5 text-teal-500" />
              </div>
              <p className="text-3xl font-bold text-foreground">${stats.monthlyTotal.toFixed(2)}</p>
              <p className="mt-1 text-xs text-muted-foreground">{t("stats.estimatedCost")}</p>
            </div>

            {/* Annual Cost */}
            <div className="rounded-lg border border-border bg-card p-5">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="text-sm font-medium text-muted-foreground">{t("stats.annual")}</h3>
                <Calendar className="h-5 w-5 text-teal-500" />
              </div>
              <p className="text-3xl font-bold text-foreground">${stats.annualTotal.toFixed(2)}</p>
              <p className="mt-1 text-xs text-muted-foreground">{t("stats.projectedSpend")}</p>
            </div>
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <p className="mt-1 text-xs text-muted-foreground">{t("stats.upcomingCharges")}</p>
            </div>
          </div>
          ...
          <div className="mb-6 rounded-lg border border-border bg-card p-4">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
              ...
              <div className="flex items-center gap-1 rounded-lg bg-muted p-1">
                {[
                  { value: "all", label: t("viewMode.all") },
                  { value: "active", label: t("viewMode.active") },
                  { value: "manual", label: t("viewMode.manual") },
                  { value: "auto-detected", label: t("viewMode.detected") },
                ].map((tab) => (
                  <button
                    key={tab.value}
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground focus:ring-2 focus:ring-teal-500"
                >
                  <option value="nextBilling">{t("sort.nextBilling")}</option>
                  <option value="name">{t("sort.name")}</option>
                  <option value="amount">{t("sort.amount")}</option>
                  <option value="createdAt">{t("sort.dateAdded")}</option>
                </select>
                <button
                  onClick={() => setSortDirection((d) => (d === "asc" ? "desc" : "asc"))}
                  className="rounded-lg border border-input p-2 transition-colors hover:bg-muted"
                  title={sortDirection === "asc" ? t("sort.descending") : t("sort.ascending")}
                >
          ...
          <div className="mt-8 rounded-lg border border-white/10 bg-slate-800/50 p-6">
            ...
            <p className="text-sm text-slate-300">{t("help.description")}</p>
          </div>
        </div>
