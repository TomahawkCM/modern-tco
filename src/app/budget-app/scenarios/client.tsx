...
import type {
  ...
  FinancialState,
  ScenarioType,
} from "@/lib/scenarios/scenario-types";
...
const SCENARIO_PRESETS: Array<{
  ...
  icon: string;
  key: string;
  inputField: keyof ScenarioInput;
}> = [
  {
    type: "cancel_subscription",
    icon: "\u{1F4B3}",
    key: "cancelSubscription",
    inputField: "subscriptionAmount",
  },
...
export function ClientScenarios() {
  ...
  const [selectedPreset, setSelectedPreset] = useState<(typeof SCENARIO_PRESETS)[number] | null>(
    null
  );
    ...
    async function loadFinancialData() {
      try {
        ...
        const totalIncome = sumAmounts(
          recentTxs.filter((tx) => tx.amount > 0).map((tx) => tx.amount)
        );
        const totalExpenses = sumAmounts(
          recentTxs.filter((tx) => tx.amount < 0).map((tx) => Math.abs(tx.amount))
        );
  ...
  const currentState: FinancialState = useMemo(() => {
    ...
    const healthScore = Math.min(
      100,
      Math.max(0, Math.round(savingsRate * 1.5 + Math.max(0, 50 - debtRatio * 50)))
    );
    return {
      monthlyIncome,
      monthlyExpenses,
          ...
          <GlassCard className="border-violet-500/20 p-5">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span
                  className={
                    cashFlow(result.after) > cashFlow(result.before)
                      ? "text-emerald-400"
                      : "text-red-400"
                  }
                >
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={
                    annualSavings(result.after) > annualSavings(result.before)
                      ? "text-emerald-400"
                      : "text-red-400"
                  }
                >
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={
                    result.after.healthScore > result.before.healthScore
                      ? "text-emerald-400"
                      : "text-red-400"
                  }
                >
