...
import { Plus, Search, Filter, Download, Edit, Trash2, Tag, FileImage, Split, Check, X as XIcon, RefreshCw, ArrowUp, ArrowDown, TrendingUp, TrendingDown } from 'lucide-react';
...
export default function TransactionsPage() {
  ...
  async function deleteTransaction(id: string) {
    ...
    try {
      ...
      if (tx?.splitFromId) {
        const siblings = await getSplitChildren(tx.splitFromId);
        if (siblings.length === 1) {
          // Last child - unsplit the parent
          const shouldRestore = await toast.confirm('This is the last split item. Restore the original transaction?');
          if (shouldRestore) {
            await unsplitTransaction(tx.splitFromId);
          } else {
  ...
  async function handleUnsplit(transaction: Transaction) {
    ...
    try {
      await unsplitTransaction(transaction.splitFromId);
      await loadData();
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 flex-1">
              <select
                ...
                className="px-4 py-2 border border-teal-300 rounded-lg bg-white focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm"
              >
        ...
        {filteredTransactions.length > 0 ? (
          <>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredTransactions.map((tx) => (
                  <tr key={tx.id} className={`hover:bg-gray-50 ${selectedTransactionIds.has(tx.id) ? 'bg-teal-50' : ''}`}>
                    <td className="sticky left-0 z-10 bg-white px-4 md:px-6 py-4 whitespace-nowrap">
                      <input
                        type="checkbox"
                    ...
                    <td className="sticky left-0 z-10 bg-white px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {new Date(tx.date).toLocaleDateString()}
                    </td>
                    <td className="px-4 md:px-6 py-4 text-sm text-gray-900 min-w-[200px]">
                      <div>
                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="ml-4 border-l-2 border-gray-200">
                                              {cat.subcategories.map(sub => (
                                                <button
                                                  ...
                                                  className="w-full text-left px-4 py-2 text-xs text-gray-600 hover:bg-gray-100 rounded"
                                                >
                                                  {sub}
                                                </button>
                                              ))}
            ...
            >
              ...
              <div className="space-y-4 p-4" style={{ marginTop: pullDistance > 0 ? `${Math.min(pullDistance, 100)}px` : 0 }}>
              {filteredTransactions.map((tx) => (
                <motion.div
                  key={tx.id}
                  drag="x"
                  ...
                  className={`bg-white border-2 rounded-lg p-4 shadow-sm transition-colors relative ${
                    selectedTransactionIds.has(tx.id) ? 'border-teal-500 bg-teal-50' : 'border-gray-200'
                  }`}
                >
                  ...
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Notes */}
                      {tx.notes && (
                        <p className="text-sm text-gray-500 mt-2">{tx.notes}</p>
                      )}
                    </div>
                    ...
                    <div className="flex-shrink-0 text-right">
                      <div className="flex items-center gap-1.5 justify-end">
                        {tx.amount > 0 ? (
                          ...
                          <>
                            <ArrowDown className="w-5 h-5 text-gray-600" aria-hidden="true" />
