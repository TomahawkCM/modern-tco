        ...
        return (
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              ...
              <Tooltip formatter={(value: number) => formatCurrencyUtil(value, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())} />
            </PieChart>
          </ResponsiveContainer>
        );
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-3xl font-bold text-green-600">
            {formatCurrencyUtil(incomeExpenses.income, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())}
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-3xl font-bold text-red-600">
            {formatCurrencyUtil(incomeExpenses.expenses, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())}
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center gap-2 text-gray-600 mb-2">
            ...
            <p className="text-sm">{t("netSavings")}</p>
          </div>
          <p className={`text-3xl font-bold ${incomeExpenses.net >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {formatCurrencyUtil(incomeExpenses.net, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())}
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center gap-2 text-gray-600 mb-2">
            ...
            <p className="text-sm">{t("savingsRate")}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t("spendingByCategory")}</h2>
          {categorySpending.length > 0 ? (
            ...
            <p className="text-gray-500 text-center py-12">{t("noSpendingData")}</p>
          )}
        </div>

        {/* Top Spending Categories - Bar Chart */}
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t("topCategories")}</h2>
          {topCategories.length > 0 ? (
            ...
            <p className="text-gray-500 text-center py-12">{t("noDataAvailable")}</p>
          )}
        </div>

        {/* Monthly Trends - Line Chart */}
        <div className="bg-white rounded-lg shadow p-6 lg:col-span-2">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t("incomeVsExpenses")}</h2>
          {monthlyTrends.length > 0 ? (
            <DynamicReportsCharts
              ...
              xKey="month"
              lines={[
                { key: 'income', name: t("income"), color: palette.positive.hex },
                { key: 'expenses', name: t("expenses"), color: palette.negative.hex },
                { key: 'net', name: t("net"), color: palette.neutral.hex },
              ]}
            />
          ) : (
            <p className="text-gray-500 text-center py-12">{t("noTrendData")}</p>
          )}
        </div>
      </div>
      ...
      <div ref={sankeyContainerRef} className="bg-slate-900 rounded-xl shadow-lg p-6 border border-white/10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">{t("moneyFlow")}</h2>
        ...
        <div className="mt-6 grid grid-cols-3 gap-4">
          <div className="rounded-lg bg-white/5 p-4 text-center">
            <p className="text-2xl font-bold text-green-400">
              {formatCurrencyUtil(sankeyData.totalIncome, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())}
            </p>
            <p className="text-xs text-slate-500 mt-1">{t("totalIncome")}</p>
          </div>
          <div className="rounded-lg bg-white/5 p-4 text-center">
            <p className="text-2xl font-bold text-red-400">
              {formatCurrencyUtil(sankeyData.totalExpenses, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())}
            </p>
            <p className="text-xs text-slate-500 mt-1">{t("totalExpenses")}</p>
          </div>
          <div className="rounded-lg bg-white/5 p-4 text-center">
            <p className={`text-2xl font-bold ${sankeyData.netSavings >= 0 ? 'text-teal-400' : 'text-red-400'}`}>
              {formatCurrencyUtil(sankeyData.netSavings, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())}
            </p>
            <p className="text-xs text-slate-500 mt-1">{t("netSavings")}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">{t("categoryBreakdown")}</h2>
        </div>
              ...
              {categorySpending.map((cat) => (
                <tr key={cat.name} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right font-semibold text-gray-900">
                    {formatCurrencyUtil(cat.value, getCurrentCurrency() || LOCALE_METADATA[getCurrentLocale()].currency, getCurrentLocale())}
                  </td>
