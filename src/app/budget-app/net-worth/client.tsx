...
export function ClientNetWorth() {
  ...
  useEffect(() => {
    async function load() {
      const [bd, snaps] = await Promise.all([
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <GlassCard className="p-5">
          ...
          <p className={`mt-1 text-3xl font-bold ${breakdown.netWorth >= 0 ? 'text-white' : 'text-red-400'}`}>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={item.label} className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-emerald-400">{fmtCurrency(item.value)}</span>
            </div>
          ))}
        </div>
      </GlassCard>
          ...
          <div className="space-y-2">
            {snapshots.slice(-12).reverse().map(snap => (
              <div key={snap.id} className="flex items-center justify-between text-sm">
                ...
                <span className={`font-medium ${snap.netWorth >= 0 ? 'text-white' : 'text-red-400'}`}>
