          ...
          {currentTx ? (
            ...
            >
              <GlassCard className="flex h-full flex-col items-center justify-center p-6 text-center">
                <p className="mb-2 text-2xl font-bold text-white">{fmtCurrency(currentTx.amount)}</p>
                <p className="text-base text-slate-300">{currentTx.description || t('noDescription')}</p>
                <p className="mt-2 text-xs text-slate-500">
                  {format.dateTime(new Date(currentTx.date), { month: 'short', day: 'numeric', year: 'numeric' })}
                </p>
              </GlassCard>
            </motion.div>
          ) : (
