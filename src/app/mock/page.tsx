...
const ExamTimer = dynamic(() => import("@/components/exam/exam-timer").then((m) => m.ExamTimer), {
  ssr: false,
  loading: () => null,
});
...
function MockExamContent() {
  ...
  useEffect(() => {
    if (state.currentSession?.completed && !reportedCompletion.current) {
      ...
      const timeSpent =
        state.currentSession.endTime && state.currentSession.startTime
          ? Math.max(
              0,
              Math.floor(
      ...
      <div className="mx-auto max-w-4xl space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mb-8 text-xl text-muted-foreground">
            Full-length timed examination simulation
          </p>
        </div>
        ...
        <Card className="glass border-white/10">
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="text-center">
                <div className="text-tanium-accent mb-2 text-2xl font-bold">105</div>
            ...
            <Alert className="border-yellow-200 bg-yellow-50/10 dark:border-yellow-800 dark:bg-yellow-900/20">
              ...
              <AlertDescription className="text-[#f97316]">
                <strong>Mock Exam Conditions:</strong> This is a timed exam that simulates real exam
                conditions. You will have 90 minutes to complete all questions. The exam will
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-foreground/80">Exam Variant:</span>
                {["A", "B", "C"].map((v) => (
                  <button
                    key={v}
                    onClick={() => {
                    ...
                    className={`rounded-md border px-3 py-1 text-sm ${variant === v ? "border-white/30 bg-white/10 text-foreground" : "border-white/20 text-foreground/80 hover:bg-white/5"}`}
        ...
        <Card className="glass border-white/10">
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              {/* Score section */}
              <div className="text-center">
                <div className="text-tanium-accent mb-2 text-5xl font-bold">{score}%</div>
                <div className="mb-4 text-muted-foreground">
                  {correctCount} out of {totalCount} correct
                </div>
                <Progress value={score} className="h-4" aria-label={`Mock exam score: ${score}%`} />
              </div>
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-[1920px] gap-6 px-4">
          ...
          <div className="pr-4">
            {currentQuestion && (
              <QuestionCard
                ...
                mode="exam"
                isMarked={
                  state.currentSession?.markedForReview?.includes(currentQuestion.id) ?? false
                }
        ...
        <div className="mt-4 flex-none">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="hidden text-xs text-muted-foreground sm:block sm:text-sm">
