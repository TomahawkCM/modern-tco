...
import { isBudget, isBudgetOffline, isTCO } from "@/config/app-target";
...
export const metadata: Metadata = isTCO
  ? {
  ...
  : {
      title: "Budget App — Personal Finance Manager",
      description: "Track spending, manage budgets, and take control of your personal finances",
      keywords: ["Budget", "Finance", "Money", "Spending", "Personal Finance"],
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <head>
        {/* PWA Manifest — only for budget targets */}
        {isBudget && (
          <>
        ...
        {/* Preconnect: Supabase — budget-online and TCO (both use Supabase) */}
        {!isBudgetOffline && (
          <>
        ...
        {/* Preconnect: YouTube + PostHog — TCO only */}
        {isTCO && (
          <>
        ...
        {/* PDF.js CDN preconnect — budget only (import feature) */}
        {isBudget && <link rel="preconnect" href="https://cdnjs.cloudflare.com" />}
      </head>
      <body
        className="layout-safe bg-background text-foreground antialiased"
        suppressHydrationWarning
      >
        {/* Load PDF.js from CDN — budget only (import feature) */}
        {isBudget && (
          <Script
        ...
        `}</style>
        {/* Register service worker — budget only (PWA) */}
        {isBudget && (
          <Script
            id="register-sw"
