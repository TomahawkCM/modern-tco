...
const DomainRadarChart = dynamic(
  () =>
    import("@/components/analytics/DomainRadarChart").then((mod) => ({
      default: mod.DomainRadarChart,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
        <div className="space-y-3 text-center">
          <div className="mx-auto h-10 w-10 animate-spin rounded-full border-b-2 border-cyan-500"></div>
...
const StudyRecommendations = dynamic(
  () =>
    import("@/components/analytics/StudyRecommendations").then((mod) => ({
      default: mod.StudyRecommendations,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const AdaptiveDifficulty = dynamic(
  () =>
    import("@/components/analytics/AdaptiveDifficulty").then((mod) => ({
      default: mod.AdaptiveDifficulty,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
...
const PerformancePredictions = dynamic(
  () =>
    import("@/components/analytics/PerformancePredictions").then((mod) => ({
      default: mod.PerformancePredictions,
    })),
  {
    loading: () => (
      <div className="flex items-center justify-center p-8">
    ...
    return {
      ...
      strongestDomain:
        domainStats.length > 0
          ? (domainStats.reduce(
              (max, domain) => (domain.score > (max?.score ?? 0) ? domain : max),
              domainStats[0]
            )?.domain ?? "None")
          : "None",
      weakestDomain:
        domainStats.length > 0
          ? (domainStats.reduce(
              (min, domain) => (domain.score < (min?.score ?? 100) ? domain : min),
              domainStats[0]
            )?.domain ?? "None")
          : "None",
    };
  ...
  const radarScores = useMemo(() => {
    ...
    return {
      ...
      [TCODomain.REFINING_QUESTIONS]: {
        score:
          (find(TCODomain.REFINING_TARGETING) || find(TCODomain.REFINING_QUESTIONS))?.score ?? 0,
        questionsAnswered:
          (find(TCODomain.REFINING_TARGETING) || find(TCODomain.REFINING_QUESTIONS))
            ?.questionsAnswered ?? 0,
        correctAnswers:
          (find(TCODomain.REFINING_TARGETING) || find(TCODomain.REFINING_QUESTIONS))
            ?.correctAnswers ?? 0,
        timeSpent:
          (find(TCODomain.REFINING_TARGETING) || find(TCODomain.REFINING_QUESTIONS))?.timeSpent ??
          0,
      },
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card className="glass border-white/10">
              ...
              <CardContent className="space-y-4">
                <div className="text-center">
                  <div className="text-tanium-accent mb-2 text-4xl font-bold">
                    {overallStats.averageScore}%
                  </div>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                <Button
                  onClick={() =>
                    router.push(
                      `/domains/${encodeURIComponent(TCODomain.NAVIGATION_MODULES.toLowerCase())}`
                    )
                  }
                ...
                <Button
                  ...
                  className="bg-tanium-accent h-auto p-4 text-foreground hover:bg-blue-600"
                >
                  <div className="text-center">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex justify-end">
                      <Button
                        size="sm"
                        onClick={() =>
                          router.push(`/domains/${encodeURIComponent(domain.domain.toLowerCase())}`)
                        }
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <DomainRadarChart
              domainScores={{
                ...
                [TCODomain.REFINING_QUESTIONS]: {
                  ...
                  correctAnswers:
                    domainStats.find((d) => d.domain === TCODomain.REFINING_QUESTIONS)
                      ?.correctAnswers ?? 0,
                  timeSpent:
                    domainStats.find((d) => d.domain === TCODomain.REFINING_QUESTIONS)?.timeSpent ??
                    0,
                },
                ...
                [TCODomain.TAKING_ACTION]: {
                  score: domainStats.find((d) => d.domain === TCODomain.TAKING_ACTION)?.score ?? 0,
                  questionsAnswered:
                    domainStats.find((d) => d.domain === TCODomain.TAKING_ACTION)
                      ?.questionsAnswered ?? 0,
                  correctAnswers:
                    domainStats.find((d) => d.domain === TCODomain.TAKING_ACTION)?.correctAnswers ??
                    0,
                  timeSpent:
                ...
                [TCODomain.NAVIGATION_MODULES]: {
                  ...
                  correctAnswers:
                    domainStats.find((d) => d.domain === TCODomain.NAVIGATION_MODULES)
                      ?.correctAnswers ?? 0,
                  timeSpent:
                    domainStats.find((d) => d.domain === TCODomain.NAVIGATION_MODULES)?.timeSpent ??
                    0,
                },
                [TCODomain.REPORTING_EXPORT]: {
                  score:
                ...
                [TCODomain.FUNDAMENTALS]: {
                  ...
                  questionsAnswered:
                    domainStats.find((d) => d.domain === TCODomain.FUNDAMENTALS)
                      ?.questionsAnswered ?? 0,
                  correctAnswers:
                    domainStats.find((d) => d.domain === TCODomain.FUNDAMENTALS)?.correctAnswers ??
                    0,
                  timeSpent:
                ...
                {recentActivity.map((session, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="bg-tanium-accent/20 flex h-10 w-10 items-center justify-center rounded-lg">
                        {session.activity.includes("Mock") ? (
                          <Clock className="text-tanium-accent h-5 w-5" />
                        ) : session.activity.includes("Domain") ? (
                          <BookOpen className="text-tanium-accent h-5 w-5" />
                        ) : (
