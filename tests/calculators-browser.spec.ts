...
test.describe('Financial Calculators Browser Tests', () => {
  ...
  test('Emergency Fund Calculator - calculates correctly', async ({ page }) => {
    ...
    const monthlyExpenses = page.locator('input[name*="expense"], input[id*="expense"], input[placeholder*="expense" i]').first();
    ...
    // Target Months: 6
    const targetMonths = page.locator('input[name*="month"], input[id*="month"], input[placeholder*="month" i]').first();
    ...
    const currentSavings = page.locator('input[name*="current"], input[id*="current"], input[name*="saving"], input[id*="saving"]').first();
    ...
    const monthlyContribution = page.locator('input[name*="contribution"], input[id*="contribution"]').first();
    ...
    console.log('Emergency Fund page content check:', pageContent?.includes('18,000') || pageContent?.includes('18000') ? '✓ Target amount displayed' : '○ Checking results display');
  ...
  test('Debt Payoff Calculator - shows snowball vs avalanche', async ({ page }) => {
    ...
    console.log(`Debt Payoff: Snowball strategy ${hasSnowball ? '✓' : '○'}, Avalanche strategy ${hasAvalanche ? '✓' : '○'}`);
  ...
  test('Subscription Cost Calculator - tracks subscriptions', async ({ page }) => {
    ...
    // Take screenshot
    await page.screenshot({ path: 'tests/screenshots/subscription-cost-breakdown.png', fullPage: true });
  ...
  test('50/30/20 Budget Analyzer - analyzes budget allocation', async ({ page }) => {
    ...
    // Take screenshot
    await page.screenshot({ path: 'tests/screenshots/budget-analyzer-50-30-20.png', fullPage: true });

    // Check for 50/30/20 references
    const pageContent = await page.textContent('body');
    const has503020 = pageContent?.includes('50') && pageContent?.includes('30') && pageContent?.includes('20');
