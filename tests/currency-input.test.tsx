...
describe('CurrencyInput', () => {
  ...
  it('renders error message with role="alert"', () => {
    render(
      <CurrencyInput value={0} onChange={() => {}} error="Amount is required" />
    );
  ...
  it('renders helper text when provided and no error', () => {
    render(
      <CurrencyInput value={0} onChange={() => {}} helperText="Enter the total" />
    );
  ...
  it('rounds to zero decimals for zero-decimal currencies (JPY)', () => {
    const onChange = vi.fn();
    render(
      <CurrencyInput value={0} onChange={onChange} currency="JPY" locale="ja-JP" />
    );
    const input = screen.getByRole('textbox');
  ...
  it('shows required indicator when required and label provided', () => {
    render(
      <CurrencyInput value={0} onChange={() => {}} label="Amount" required />
    );
    expect(screen.getByText('*')).toBeInTheDocument();
  });
});
