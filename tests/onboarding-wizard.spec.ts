...
test.describe("Onboarding Wizard - Complete Test Suite", () => {
  ...
  test("Test 1.4: Accessibility Step", async ({ page }) => {
    ...
    const accessibilityContent = page.getByText(
      /Accessibility|Easy Read|Standard|Comfortable|High Contrast/i
    );
  ...
  test("Test 2.1: Dashboard loads without errors", async ({ page }) => {
    ...
    const criticalErrors = consoleErrors.filter(
      (e) => !e.includes("Warning") && !e.includes("hydration") && !e.includes("PostHog")
    );

    console.log(
      `Found ${consoleErrors.length} console messages, ${criticalErrors.length} critical errors`
    );
