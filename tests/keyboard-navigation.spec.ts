...
test.describe('Budget App - Keyboard Navigation (Task 2.2.3)', () => {
  ...
  test('should trap focus within modal and close with Escape', async ({ page }) => {
    ...
    // Modal should close
    await expect(page.getByRole('heading', { name: /add transaction/i })).not.toBeVisible({ timeout: 2000 });
  });
  ...
  test('should manage budgets using keyboard only', async ({ page }) => {
    ...
    // Modal should close
    await expect(page.getByRole('heading', { name: /add budget|create budget/i })).not.toBeVisible();
  });
