...
async function uploadFile(page: Page, filePath: string, mimeType: string) {
...
test.describe("Multi-Format Import Pipeline", () => {
    ...
    test.describe(`PDF - ${pdfTest.language}`, () => {
      test(`should import ${pdfTest.language} PDF bank statement`, async ({ page }) => {
        ...
        try {
          ...
          await expect(page.locator(`text=/${pdfTest.file}/i`).first()).toBeVisible({
            timeout: 10000,
          });

          // Check format was detected as PDF
          await expect(page.locator("text=/PDF|pdf/i").first()).toBeVisible({ timeout: 5000 });
  ...
  test.describe("QIF Format", () => {
    test("should import QIF checking account file", async ({ page }) => {
      ...
      try {
        ...
        await expect(page.locator("text=/checking-sample.qif/i").first()).toBeVisible({
          timeout: 10000,
        });
  ...
  test.describe("MT940 Format", () => {
    test("should import MT940 SWIFT bank statement", async ({ page }) => {
      ...
      try {
        ...
        await expect(page.locator("text=/deutsche-bank-sample.sta/i").first()).toBeVisible({
          timeout: 10000,
        });
  ...
  test.describe("CAMT.053 Format", () => {
    test("should import CAMT.053 ISO 20022 XML statement", async ({ page }) => {
      ...
      try {
        ...
        await expect(page.locator("text=/iso20022-sample.xml/i").first()).toBeVisible({
          timeout: 10000,
        });
  ...
  test.describe("Error Handling", () => {
    ...
    test("should handle invalid XML for CAMT.053", async ({ page }) => {
      ...
      try {
        const invalidXml =
          '<?xml version="1.0"?><Document><BkToCstmrStmt><Stmt></Stmt></BkToCstmrStmt></Document>';
  ...
  test.describe("Format Detection", () => {
    test("should detect PDF format from file", async ({ page }) => {
      ...
      await expect(page.locator("text=/PDF/i").first()).toBeVisible({ timeout: 10000 });
    });
