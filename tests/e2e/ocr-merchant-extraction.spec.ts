...
async function dismissAllOverlays(page: Page) {
  ...
  await page.evaluate(() => {
    localStorage.setItem('budget-app-tour-progress', JSON.stringify({
      completed: true, currentStep: 999, completedAt: Date.now(),
    }));
  });
...
test.describe('OCR Merchant Extraction', () => {
  ...
  test('uploads bank statement PDF and extracts merchant via OCR', async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        const input = document.querySelector('input[placeholder="e.g., Walmart, Starbucks"]') as HTMLInputElement;
    ...
    if (matchedKnown) {
      ...
      console.log(`Got merchant: "${merchantValue}" (OCR quality may vary — known merchant match not required)`);
    }
