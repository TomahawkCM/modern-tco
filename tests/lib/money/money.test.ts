...
describe("Money Utilities Module", () => {
  ...
  describe("roundToCents()", () => {
    ...
    it("should handle already-rounded values", () => {
      ...
      expect(roundToCents(100.0)).toBe(100);
    });
  ...
  describe("Real-world Scenarios", () => {
    it("should calculate correct monthly total from transactions", () => {
      ...
      const income = sumAmounts(transactions.filter((tx) => tx.amount > 0).map((tx) => tx.amount));
      const expenses = Math.abs(
        sumAmounts(transactions.filter((tx) => tx.amount < 0).map((tx) => tx.amount))
      );
    ...
    it("should handle budget calculations with overspending", () => {
      ...
      const expenses = [89.99, 125.5, 200.0, 45.75, 50.25];
