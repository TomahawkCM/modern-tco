...
describe('refund-tracker', () => {
  ...
  describe('markExpectingRefund', () => {
    ...
    it('works without optional expectedDate and notes', async () => {
      await seedTransaction({ id: 'tx-r2', amount: -50.00, description: 'WALMART item' });

      await markExpectingRefund('tx-r2', 50.00);

      const tx = await db.transactions.get('tx-r2');
      expect(tx!.refundStatus).toBe('expecting');
      expect(tx!.refundExpectedAmount).toBe(50.00);
  ...
  describe('autoMatchRefunds', () => {
    ...
    it('detects partial refund when credit is less than 90% of purchase', async () => {
      ...
      expect(purchase!.refundStatus).toBe('received');
      expect(purchase!.refundReceivedAmount).toBe(180.00);
    });
  ...
  describe('getRefundSummary', () => {
    it('calculates correct gross spending, refunds, and net', async () => {
      ...
      await seedTransaction({ id: 'tx-s1', amount: -150.00, date: mid, description: 'Groceries' });
      await seedTransaction({ id: 'tx-s2', amount: -200.00, date: mid, description: 'Electronics' });
      await seedTransaction({ id: 'tx-s3', amount: -50.00, date: mid, description: 'Coffee' });
      ...
      expect(summary.gross).toBe(400.00);
      // Refunds = sum of positive amounts with refundLinkedTransactionId = 75
      expect(summary.refunds).toBe(75.00);
      // Net = gross - refunds = 400 - 75 = 325
      expect(summary.net).toBe(325.00);
    });
    ...
    it('excludes split parent transactions', async () => {
      ...
      expect(summary.gross).toBe(300.00);
    });
    ...
    it('handles floating point precision with rounding', async () => {
      ...
      expect(summary.gross).toBe(100.00);
      expect(summary.net).toBe(100.00);
    });
  });
