...
CREATE TABLE public.user_study_bookmarks (
    ...
    position INTEGER NOT NULL DEFAULT 0, -- Character position in content
    note TEXT,
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
...
CREATE INDEX idx_study_modules_domain ON public.study_modules(domain);
CREATE INDEX idx_study_sections_module_id ON public.study_sections(module_id);
CREATE INDEX idx_study_sections_module_order ON public.study_sections(module_id, order_index);
CREATE INDEX idx_user_study_progress_user_id ON public.user_study_progress(user_id);
CREATE INDEX idx_user_study_progress_module_id ON public.user_study_progress(module_id);
CREATE INDEX idx_user_study_progress_status ON public.user_study_progress(status);
CREATE INDEX idx_user_study_bookmarks_user_id ON public.user_study_bookmarks(user_id);
CREATE INDEX idx_user_study_bookmarks_section_id ON public.user_study_bookmarks(section_id);
...
-- Study modules and sections are public (readable by all authenticated users)
...
-- User progress and bookmarks are private to each user
...
-- Functions to maintain updated_at timestamps
...
-- Grant permissions for the service role to manage study content
