...
function runSingleSimulation(
  ...
  for (let year = 1; year <= input.years; year++) {
    for (let month = 0; month < 12; month++) {
      ...
      const adjustedWithdrawal = input.monthlyWithdrawal * inflationFactor;
      balance = Math.max(0, balance - adjustedWithdrawal);
    }

    yearEndBalances.push(round2(balance));
  }
...
export function runMonteCarlo(input: MonteCarloInput): MonteCarloResult {
  ...
  for (let year = 0; year <= input.years; year++) {
    const balancesAtYear = allSimulations
      .map((sim) => sim[year]!)
      .sort((a, b) => a - b);

    percentiles.push({
      year,
      p10: round2(percentile(balancesAtYear, 10)),
      p25: round2(percentile(balancesAtYear, 25)),
      p50: round2(percentile(balancesAtYear, 50)),
      p75: round2(percentile(balancesAtYear, 75)),
      p90: round2(percentile(balancesAtYear, 90)),
    });
  }

  // Success rate (balance > 0 at final year)
  const finalBalances = allSimulations
    .map((sim) => sim[input.years]!)
    .sort((a, b) => a - b);
  ...
  return {
    ...
    failureCount,
    medianFinalBalance: round2(percentile(finalBalances, 50)),
