...
-- New users are automatically inserted via trigger on auth.users
-- So we don't need INSERT policy for regular users
...
-- (This would require an admin role system - for now, allow updates by creator)
